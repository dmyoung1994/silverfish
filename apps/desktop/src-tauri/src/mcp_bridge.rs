...
fn project_config(workspace: &str) -> Option<PathBuf> {
    (!workspace.trim().is_empty())
        .then(|| PathBuf::from(workspace).join(".silverfish/mcp-servers.json"))
}
...
fn isolated_codex_home(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    std::fs::create_dir_all(&root)
        .map_err(|error| format!("Could not prepare the Silverfish Codex runtime: {error}"))?;
    let Some(host_home) = codex_home() else {
        return Ok(root);
    };
...
fn link_if_missing(source: &PathBuf, destination: &PathBuf) -> Result<(), String> {
    ...
    std::os::unix::fs::symlink(source, destination).map_err(|error| {
        format!("Could not link {source:?} into the Silverfish Codex runtime: {error}")
    })?;
    #[cfg(not(unix))]
    std::fs::copy(source, destination).map_err(|error| {
        format!("Could not copy {source:?} into the Silverfish Codex runtime: {error}")
    })?;
    Ok(())
}
