...
impl CodexClient {
    pub async fn spawn(
        ...
        command
            .args([
                "-c",
                &format!("mcp_servers.{}.command={:?}", bridge.name, "node"),
            ])
            .args([
                "-c",
                &format!(
                    "mcp_servers.{}.args={:?}",
                    bridge.name,
                    vec![bridge.script.to_string_lossy().to_string()]
                ),
            ])
            .args([
                "-c",
                &format!(
                    "mcp_servers.{}.env={{SILVERFISH_MCP_BRIDGE_CONFIG={:?}}}",
                    bridge.name,
                    bridge.upstream_config.to_string_lossy()
                ),
            ])
...
pub async fn install_skill_from_github(
    ...
    if !source.starts_with("https://github.com/") {
        return Err(
            "Enter a GitHub skill URL (https://github.com/owner/repo/tree/ref/path).".into(),
        );
    }
    let codex_home =
        codex_home().ok_or_else(|| "Could not find the host Codex home directory.".to_owned())?;
    let installer =
        codex_home.join("skills/.system/skill-installer/scripts/install-skill-from-github.py");
    if !installer.is_file() {
        return Err(
            "The host Codex skill installer is unavailable. Update Codex, then try again.".into(),
        );
    }
    ...
    if !output.status.success() {
        ...
        return Err(if detail.is_empty() {
            "Codex could not install that skill.".into()
        } else {
            detail
        });
    }
...
fn discover_skills(workspace: Option<&str>, agent: &str) -> Vec<AgentSkill> {
    ...
    let claude_home =
        std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".claude/skills"));
    ...
    for (root, source) in roots {
        for manifest in skill_manifests(&root) {
            let Some(mut skill) = parse_skill_manifest(&manifest, source) else {
                continue;
            };
