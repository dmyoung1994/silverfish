...
pub async fn detect_status() -> ClaudeStatus {
    ...
    ClaudeStatus {
        ...
        version,
        approval_mediated: version_output
            .as_ref()
            .is_some_and(|output| output.status.success()),
    }
}
...
impl ClaudeClient {
    ...
    pub async fn start_turn(&self, text: &str) -> Result<(), String> {
        ...
        let executable =
            find_claude_executable().ok_or_else(|| "Claude Code is not installed".to_owned())?;
        ...
        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| "Claude Code did not expose stdout".to_owned())?;
        ...
        *self.active.lock().await = Some(child);
        ...
        let _ = self.events.send(
            json!({ "method": "turn/started", "params": { "turn": { "id": "claude-turn" } } }),
        );
        Ok(())
    }
    ...
    pub async fn resolve_approval(&self, request_id: Value, decision: &str) -> Result<(), String> {
        let behavior = if decision == "approveOnce" {
            "allow"
        } else {
            "deny"
        };
        ...
        let stdin = stdin
            .as_mut()
            .ok_or_else(|| "Claude Code is not waiting for an approval".to_owned())?;
...
fn normalize_event(value: &Value) -> Vec<Value> {
    ...
    if event_type == "stream_event" {
        ...
        if let Some(delta) = event.pointer("/delta/text").and_then(Value::as_str) {
            return vec![
                json!({ "method": "item/agentMessage/delta", "params": { "itemId": "claude-message", "delta": delta } }),
            ];
        }
    }
    if event_type == "assistant" {
        ...
        if !text.is_empty() {
            return vec![
                json!({ "method": "item/completed", "params": { "itemId": "claude-message", "item": { "type": "agentMessage", "text": text } } }),
            ];
        }
    }
...
fn find_claude_executable() -> Option<PathBuf> {
    ...
    if let Some(path) = std::env::var_os(CLAUDE_PATH_ENV) {
